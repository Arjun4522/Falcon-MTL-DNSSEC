...
size_t base64_decode_openssl(const char *input, unsigned char *output, size_t max_len) {
    BIO *bio, *b64;
    int decode_len;
    
    char *clean_input = malloc(strlen(input) + 1);
    if (!clean_input) {
        fprintf(stderr, "Memory allocation failed for clean_input\n");
        return 0;
    }
    ...
    printf("Base64 input (first 64 chars): %.64s\n", clean_input);
    ...
    bio = BIO_new_mem_buf(clean_input, -1);
    if (!bio) {
    ...
    decode_len = BIO_read(bio, output, max_len);
...
size_t base64_decode_manual(const char *input, unsigned char *output, size_t max_len) {
    ...
    for (size_t i = 0; i < input_len; i += 4) {
        ...
        for (int j = 0; j < 64; j++) {
            if (i < input_len && b64[j] == input[i]) a = j;
            if (i + 1 < input_len && b64[j] == input[i + 1]) b = j;
            if (i + 2 < input_len && b64[j] == input[i + 2]) c = j;
            if (i + 3 < input_len && b64[j] == input[i + 3]) d = j;
        }
        
        if (output_len + 3 > max_len) break;
        
        output[output_len++] = (a << 2) | (b >> 4);
        if (i + 2 < input_len) output[output_len++] = ((b & 0x0F) << 4) | (c >> 2);
        if (i + 3 < input_len) output[output_len++] = ((c & 0x03) << 6) | d;
    }
...
size_t load_key(const char *filename, unsigned char *key, size_t max_len) {
    ...
    if (len > max_len) {
        fprintf(stderr, "Key %s too large (%zu bytes, max %zu)\n", filename, len, max_len);
        fclose(f);
        return 0;
    }
...
char *load_signature(const char *filename) {
    ...
    size_t bytes_read = fread(sig, 1, file_size, f);
    sig[bytes_read] = '\0';
    
    while (bytes_read > 0 && (sig[bytes_read - 1] == '\n' || sig[bytes_read - 1] == '\r')) {
        sig[--bytes_read] = '\0';
    }
    ...
    *output_len = 0;
    for (int i = 0; i < rrset_count; i++) {
        ...
        if (*output_len + len + 1 > 4096) {
            fprintf(stderr, "Canonical RRset buffer overflow\n");
            for (int j = 0; j < rrset_count; j++) free(sorted_rrset[j]);
            free(sorted_rrset);
            exit(1);
        }
...
void canonicalize_dnskey_rrset(const unsigned char *ksk_pubkey, size_t ksk_len,
    ...
    *output_len = 0;
    for (int i = 0; i < 2; i++) {
        ...
        if (*output_len + len + 1 > 4096) {
            fprintf(stderr, "Canonical DNSKEY buffer overflow\n");
            exit(1);
        }
...
int main(int argc, char *argv[]) {
    printf("DNSSEC Resolver Verification for Falcon-512 (Fixed)\n");
    printf("==================================================\n\n");
    ...
    if (ksk.pubkey_len != FALCON512_PUBLIC_KEY_SIZE || zsk.pubkey_len != FALCON512_PUBLIC_KEY_SIZE) {
        fprintf(stderr, "Invalid key sizes (KSK: %zu, ZSK: %zu)\n", ksk.pubkey_len, zsk.pubkey_len);
        return 1;
    }
    ...
    printf("\n=== Signature Decoding Debug ===\n");
    printf("Expected signature size: %d bytes\n", FALCON512_SIGNATURE_SIZE);
    
    size_t a_sig_len = base64_decode_openssl(a_sig_base64, a_sig, FALCON512_SIGNATURE_SIZE);
    if (a_sig_len == 0) {
        printf("OpenSSL decoder failed, trying manual decoder...\n");
        a_sig_len = base64_decode_manual(a_sig_base64, a_sig, FALCON512_SIGNATURE_SIZE);
    }
    
    free(a_sig_base64);
    
    if (a_sig_len == 0) {
        fprintf(stderr, "Base64 decoding failed completely\n");
        free_rrset(rrset, rrset_count);
        return 1;
    }
    
    if (a_sig_len != FALCON512_SIGNATURE_SIZE) {
        fprintf(stderr, "Invalid signature length: %zu (expected %d)\n", a_sig_len, FALCON512_SIGNATURE_SIZE);
        free_rrset(rrset, rrset_count);
        return 1;
    }
    
    printf("Signature decoded successfully: %zu bytes\n", a_sig_len);

    // Verify A/AAAA/TXT signature
    unsigned char tmp[FALCON512_VERIFY_TMP_SIZE];
    printf("\n=== Signature Verification ===\n");
    int verify_result = falcon_verify(a_sig, a_sig_len, zsk.pubkey, zsk.pubkey_len,
                                     rrsig_data_a, rrsig_data_a_len, tmp, sizeof(tmp));
    if (verify_result == 0) {
    ...
    canonicalize_dnskey_rrset(ksk.pubkey, ksk.pubkey_len, zsk.pubkey, zsk.pubkey_len,
                              owner, 3600, canonical_dnskey, &canonical_dnskey_len);
    printf("\nCanonicalized DNSKEY RRset:\n%.*s\n", (int)canonical_dnskey_len, canonical_dnskey);
    ...
    to_hex(dnskey_hash, 32, hash_hex);
    printf("DNSKEY RRSIG rr_hash: %s\n", hash_hex);
    ...
    rrsig_data_dnskey_len += 2;
    strcpy((char *)rrsig_data_dnskey + rrsig_data_dnskey_len, signer_name);
    rrsig_data_dnskey_len += strlen(signer_name) + 1;
    ...
    if (!dnskey_sig_base64) {
        free_rrset(rrset, rrset_count);
        return 1;
    }
    unsigned char dnskey_sig[FALCON512_SIGNATURE_SIZE];
    
    printf("\n=== DNSKEY Signature Decoding Debug ===\n");
    printf("Expected signature size: %d bytes\n", FALCON512_SIGNATURE_SIZE);
    
    size_t dnskey_sig_len = base64_decode_openssl(dnskey_sig_base64, dnskey_sig, FALCON512_SIGNATURE_SIZE);
    if (dnskey_sig_len == 0) {
        printf("OpenSSL decoder failed, trying manual decoder...\n");
        dnskey_sig_len = base64_decode_manual(dnskey_sig_base64, dnskey_sig, FALCON512_SIGNATURE_SIZE);
    }
    
    free(dnskey_sig_base64);
    
    if (dnskey_sig_len == 0) {
        fprintf(stderr, "DNSKEY Base64 decoding failed completely\n");
        free_rrset(rrset, rrset_count);
        return 1;
    }
    
    if (dnskey_sig_len != FALCON512_SIGNATURE_SIZE) {
        fprintf(stderr, "Invalid DNSKEY signature length: %zu (expected %d)\n", dnskey_sig_len, FALCON512_SIGNATURE_SIZE);
        free_rrset(rrset, rrset_count);
        return 1;
    }
    
    printf("DNSKEY signature decoded successfully: %zu bytes\n", dnskey_sig_len);

    // Verify DNSKEY signature
    verify_result = falcon_verify(dnskey_sig, dnskey_sig_len, ksk.pubkey, ksk.pubkey_len,
                                  rrsig_data_dnskey, rrsig_data_dnskey_len, tmp, sizeof(tmp));
    if (verify_result == 0) {
    ...
    // Cleanup
    free_rrset(rrset, rrset_count);
    printf("\nDNSSEC verification completed successfully\n");
    return 0;
}