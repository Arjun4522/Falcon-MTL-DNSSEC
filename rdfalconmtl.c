...
void save_output(const char *label, const char *value) {
    fprintf(output_file, "%s: %s\n", label, value);
    printf("%s: %s\n", label, value);
}

// Save binary data as hex to output file
void save_binary_output(const char *label, const unsigned char *data, size_t len) {
    char *hex_str = malloc(len * 2 + 1);
    hash_to_string(data, len, hex_str);
    save_output(label, hex_str);
    free(hex_str);
}
...
void generate_and_sign_merkle_root() {
    ...
    // Open output file
    output_file = fopen("merkle_falcon_outputs.txt", "w");
    if (!output_file) {
        fprintf(stderr, "Failed to open output file\n");
        return;
    }

    fprintf(output_file, "Falcon-512 Merkle Tree Outputs\n");
    fprintf(output_file, "==============================\n\n");

    // Generate key pairs
    fprintf(output_file, "Generated %d Falcon-512 key pairs:\n", num_keys);
    ...
    for (int i = 0; i < num_keys; i++) {
        ...
        char label[32];
        sprintf(label, "Key %d PubKey Hash", i);
        save_output(label, pubkey_hash_str);
    }
    ...
    // Save signature (full signature)
    char sig_full_str[FALCON512_SIGNATURE_SIZE * 2 + 1];
    hash_to_string(signature, FALCON512_SIGNATURE_SIZE, sig_full_str);
    save_output("\nSignature (full)", sig_full_str);
    ...
    for (int i = 0; i < tree.levels_count - 1; i++) {
        if (proof[i]) {
            ...
            save_output(label, proof_str);
        } else {
            char label[32];
            sprintf(label, "Level %d", i);
            save_output(label, "(no sibling)");
        }
    }
...
int main() {
    ...
    printf("\nAll outputs have been saved to 'merkle_falcon_outputs.txt'\n");
    return 0;
}
